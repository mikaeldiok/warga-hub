...
class UnitsImport implements ToCollection, WithHeadingRow
{
    ...
    public function collection(Collection $rows)
    {
        ...
        foreach ($rows as $row) {
            ...
            if($zip){
                $photoExist = $zip->has($row['unit_id'].".jpg", ZipArchive::FL_NOCASE|ZipArchive::FL_NODIR);
                
                // \Log::debug($row['unit_id']."jpeg");
                // \Log::debug($zip->has($row['unit_id'].".jpeg", ZipArchive::FL_NOCASE|ZipArchive::FL_NODIR));
                // \Log::debug($zip->has($row['unit_id'].".jpg", ZipArchive::FL_NOCASE|ZipArchive::FL_NODIR));
                // \Log::debug($zip->has($row['unit_id'], ZipArchive::FL_NOCASE|ZipArchive::FL_NODIR));
                
                if($photoExist){
                    $isExtracted = $zip->extract(storage_path('app/ziptmp'), $row['unit_id'].".jpg");

                    if($isExtracted){
                        $photo =  \Storage::get('ziptmp/'.$row['unit_id'].".jpg");
                        if ($unit->getMedia($this->module_name)->first()) {
                            $unit->getMedia($this->module_name)->first()->delete();
                        }
            
                        $media = $unit->addMediaFromDisk('ziptmp/'.$row['unit_id'].".jpg",'local')->toMediaCollection($this->module_name);
