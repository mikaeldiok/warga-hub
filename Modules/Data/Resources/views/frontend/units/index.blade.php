            ...
            <div class="card bg-white border-light shadow flex-md-row no-gutters p-4">
                <div class="card-body d-flex flex-column justify-content-between col-auto py-3">
                    <form name="filterForm" id="filterForm" method="post" action="javascript:void(0)">
                        ...
                        {{ method_field('POST') }}
                        @include('data::frontend.units.filter-form')
        ...
        <div class="col-lg-9 mb-5">
            <div class="card bg-white border-light shadow flex-md-row no-gutters p-4">
                <div class="card-body d-flex flex-column justify-content-between col-auto py-4 px-2">
                    @if (count($units) > 0)
                        <section id="units">
                            @include('data::frontend.units.units-card-loader')
                        </section>
                    @endif
                </div>
            </div>
...
<script type="text/javascript">
    $(function() {
        $('body').on('click', '.pagination a', function(e) {
            ...
            $('#units a').css('color', '#dfecf6');
            $('#units').append('<img style="position: absolute; left: 0; top: 0; z-index: 100000;" src="/images/loading.gif" />');
    ...
    $(function() {
        $('body').on('click', '#clearFilter', function(e) {
            ...
            $.ajax({
                url: "{{route('frontend.units.filterUnits')}}",
                type: "GET",
                data: null,
