            ...
            <div class="card bg-white border-light shadow flex-md-row no-gutters p-4">
                <div class="card-body d-flex flex-column justify-content-between col-auto py-3">
                    <form name="filterForm" id="filterForm" method="post" action="javascript:void(0)">
                        ...
                        {{ method_field('POST') }}
                        @include('performance::frontend.parameters.filter-form')
                ...
                <div class="card-body d-flex flex-column justify-content-between col-auto py-4 px-2">
                    @if (count($parameters) > 0)
                        <section id="parameters">
                            @include('performance::frontend.parameters.parameters-card-loader')
                        </section>
                    @endif
                </div>
            </div>
...
<script type="text/javascript">
    $(function() {
        $('body').on('click', '.pagination a', function(e) {
            ...
            $('#parameters a').css('color', '#dfecf6');
            $('#parameters').append('<img style="position: absolute; left: 0; top: 0; z-index: 100000;" src="/images/loading.gif" />');
        ...
        function getArticles(url) {
            $.ajax({
                ...
                alert('Parameters could not be loaded.');
            });
        }
    });
    ...
    $(function() {
        $('body').on('click', '#clearFilter', function(e) {
            ...
            $.ajax({
                url: "{{route('frontend.parameters.filterParameters')}}",
                type: "GET",
                data: null,
